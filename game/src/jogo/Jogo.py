...
from entidade.defesas.impl import TorreNormal, TorreSniper, TorreRapida, TorreVaga
...
class Jogo:
    def __init__(self, nome_jogador: str, database: Database):
        ...
        self.inimigos = []
        self.__configurar_torres_vagas()
        self.projeteis = []
    ...
    def __configurar_torres_vagas(self):
        self.torres = []
        for torre in TORRES:
            self.torres.append(TorreVaga(torre[0], torre[1]))
    ...
    def construir_torre(self):
        ...
        if self.dinheiro >= tipo_torre["custo"]:
            for torre in self.torres:
                if isinstance(torre, TorreVaga):
                    if torre.esta_na_area(mx, my):
                        tx = torre.x
                        ty = torre.y
                        self.torres.remove(torre)
                        torre = tipo_torre["classe"](tx, ty)
    ...
    def remover_torre(self):
        ...
        for torre in self.torres:
            if not isinstance(torre, TorreVaga):
                if torre.esta_na_area(mx, my):
                    tx = torre.x
                    ty = torre.y
                    self.torres.remove(torre)
                    torre_nova = TorreVaga(tx, ty)
                    self.torres.append(torre_nova)
                    dinheiro = tipo_torre["custo"] * 0.6
                    self.dinheiro += dinheiro
                    self.player.adicionar_dinheiro(dinheiro)
                    print(f"Torre {tipo_torre['nome']} destruída por {self.nome_jogador}!")
